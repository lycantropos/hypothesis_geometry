...
from .hints import Multicontour, Orienteer, ScalarT
...
def has_horizontal_lowermost_segment(
    ...
    return is_segment_horizontal(lowermost_segment) or any(
        segment_to_min_y(segment) == min_y and is_segment_horizontal(segment)
...
def has_vertical_leftmost_segment(
    ...
    return is_segment_vertical(leftmost_segment) or any(
        segment_to_max_x(segment) == max_x and is_segment_vertical(segment)
        for segment in segments
    )
...
def multicontour_has_valid_sizes(
    ...
    *,
    ...
    return has_valid_size(
        multicontour, min_size=min_size, max_size=max_size
    ) and all(
        has_valid_size(
            contour.vertices,
...
def angle_contains_point(
    ...
    *,
    ...
    return (
        second_half_orientation is angle_orientation
        if first_half_orientation is Orientation.COLLINEAR
        else (
            first_half_orientation is angle_orientation
...
def are_vertices_non_convex(
    ...
    return any(
        orientation is not base_orientation for orientation in orientations
    )
